...
- `approval` 必须写 `transitions.approve` / `transitions.reject`
...
- 节点 `contextDir` 优先级最高：节点级 `contextDir` > workflow 级 `contextDir` > 当前节点最终解析出来的 `workdir`。
...
`codex` 节点支持 `reentry` 配置，用于控制同一次 run 内再次进入该节点时如何打开终端。
