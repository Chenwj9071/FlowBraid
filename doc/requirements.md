...
- `workdir`：共享业务目录，节点真实修改和验证的业务文件放在这里。
...
- `nodes/<node-id>/state/control-log.jsonl`：任务型节点控制事件唯一真源，记录 attempt 生命周期与终态语义。
- `nodes/<node-id>/state/runtime-state.json`：由控制真源派生的最新运行态快照，记录 `status`、`outcome`、`summary`、`attemptId` 等信息。
...
- native split 下的 `codex` 节点如果长时间等待用户输入，不应仅因耗时较长被判定失败；只有终端或会话明确失联时，才进入人工处理分支。
- native split 下的 `codex` 节点在终端失联后，应默认进入人工确认等待，由用户通过 `resume` 选择重试当前节点或继续后续节点。
- 对会回流的 `codex` 节点，新 prompt 约定应显式暴露验证报告和人工反馈路径，避免依赖隐式文件约定。
- 当主调度器异常退出、终端误关或 run 进入不一致状态时，必须可以通过 `recover` 重建控制权并继续流程或人工决策。
