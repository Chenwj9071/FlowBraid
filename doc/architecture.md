...
- 调度层：负责驱动节点执行，处理暂停、审批、回流、`resume` 和 `send`。
...
- `codex` 的交互模式通过 PTY 直连当前终端，输入输出走同一条 terminal 通道。
- Windows 下的 PTY 链路会先切到 UTF-8 控制台，再启动 `codex`，减少中文文档和人工输入乱码问题。
- `agent_session` 节点不再把“进程退出”当作完成信号，而是依赖 provider 单次 `turn` 的结构化结果。
- 当前 provider 只落地了 `codex`，但接口边界已经按 `agent_session` 拆开，后续可接入其他 provider。
...
- `codex` / `agent_session` 启动时同时拿到两类目录：
